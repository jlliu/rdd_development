var gameModeCanv = function (p) {
  ...
  p.setup = function () {
    ...
    gameModeCanvas = p.createCanvas(canvasWidth, canvasHeight).elt;
    gameModeCanvas.classList.add("gameCanvas");
    gameModeCanvas.id = "gameModeCanvas";
    ...
    menuItems = [
      new menuItem("STORY MODE", null, 70, selectMenuItem),
      new menuItem("ARCADE MODE", null, 150, selectMenuItem),
      new menuItem("OPTIONS", null, 230, selectMenuItem),
      new menuItem("CREDITS", null, 310, selectMenuItem),
    ];

    window.dispatchEvent(canvasLoadedEvent);

    setupNavigation(gameModeCanvas);
  };
  ...
  function selectMenuItem(option) {
    ...
    if (option == "STORY MODE") {
      ...
      gameModeCanvas.dispatchEvent(hideSceneEvent);
      sound_fx.select.start();
    }
    if (option == "ARCADE MODE") {
      gameMode = "Arcade";
      difficultyCanvas.dispatchEvent(showSceneEvent);
      gameModeCanvas.dispatchEvent(hideSceneEvent);
      sound_fx.select.start();
...
new p5(gameModeCanv, "game-mode-canvas-container");
